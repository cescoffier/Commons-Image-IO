...
public class IptcParser extends BinaryFileParser implements IptcConstants
{
            ...
            throws ImageReadException, IOException
    {
        ...
        while (index + 1 < bytes.length)
        {
            ...
            {
                if (verbose)
                    Debug.debug("Unexpected record tag marker in IPTC data - Ignoring record");
                //CE: Do not return here. Let's try to check the others blocks.
                continue;
            }
        ...
        byte blockData[];
        {
            ...
            for (int i = 0; i < elements.size(); i++)
            {
                ...
                if (element.value == null) {
                    System.err.println("No value for " + element.iptcType.getName());
                    continue;
                }
